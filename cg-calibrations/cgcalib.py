...
def parse_image_path(path):
    ...
    csv = pandas.read_csv("sequences.csv", parse_dates=['first_time_utc', 'last_time_utc'])
...
def find_image(path, root="."):
    ...
    if os.path.isdir(service_path):
        ...
        for subdir in subdirs:
            image_path = os.path.join(service_path, subdir,
                os.path.splitext(os.path.basename(path))[0] + ".JPG")
            if os.path.isfile(image_path):
                return image_path

def station_eop(station):
    geo = helper.read_geojson("geojson/stations.geojson", crs=32606, key="id")
    return dict(
...
def gcp_points(img, markup):
    ...
    geo = helper.read_geojson("geojson/gcp.geojson", key="id", crs=32606)
...
def coast_lines(img, markup):
    luv = markup.values()
    geo = helper.read_geojson("geojson/coast.geojson", crs=32606)
    lxy = [feature['geometry']['coordinates'] for feature in geo['features']]
    lxyz = [np.hstack((xy, sea_height(xy, datetime=img.datetime))) for xy in lxy]
    return optimize.Lines(img.cam, luv, lxyz)

def terminus_lines(img, markup):
    luv = markup.values()
    geo = helper.read_geojson("geojson/termini.geojson", key="date", crs=32606)
    date_str = img.datetime.strftime("%Y-%m-%d")
    xy = geo['features'][date_str]['geometry']['coordinates']
    xyz = np.hstack((xy, sea_height(xy, datetime=img.datetime)))
...
def moraines_mlines(img, markup):
    date_str = img.datetime.strftime("%Y%m%d")
    geo = helper.read_geojson("geojson/moraines/" + date_str + ".geojson", key="id", crs=32606)
    mlines = []
...
def sea_height(xy, crs=32606, datetime=None):
    # TODO: Use locally solved geoid-ellipsoid offset surface or GEOID96
    # TODO: Correct for tidal variation
    return np.repeat(17, len(xy)).reshape(-1, 1)

def load_calibration(path, station=True, camera=True, **kwargs):
    calibration = dict()
    ids = parse_image_path(path)
    if station:
        station_path = "stations/" + ids['station'] + ".json"
        calibration = helper.merge_dicts(
            calibration, helper.read_json(station_path))
    else:
        eop = station_eop(ids['station'])
        calibration = helper.merge_dicts(
            calibration, dict(xyz=eop['xyz'], viewdir=eop['viewdir']))
    if camera:
        camera_path = "cameras/" + ids['camera'] + ".json"
        calibration = helper.merge_dicts(
            calibration, helper.read_json(camera_path))
...
def camera_motion_matches(camera, root=".", size=1, force_size=False, method="sift",
    ...
    motion = helper.read_json("motion.json")
